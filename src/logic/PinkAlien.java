...
public class PinkAlien extends Enemy {

	private static final int LIFE = 2;
	private static final int SPEED = 1;
	private static final int INCROCKET = 150;
	private static final int SCORE = 5;

	private static final int delayWalk = 10;
	private int countDelayWalk = 0;
	private int countFrameWalk = 0;

	private static final int delayClimb = 14;
	...
	private static final int postYD = ConfigurableOption.screenHeight - 133;
	private static final int postYU = ConfigurableOption.screenHeight - 251;
	private static final int postLadderL = 125;
	private static final int postLadderR = 546;
	...
	public PinkAlien(Position post, int z, int time_release, int wave,
	...
	public void animationWalkL(Graphics2D g2d, int postX, int postY) {
		...
		if (countFrameWalk == 0) {
			g2d.drawImage(DrawingUtility.pinkWalkL1, null, postX - speedWalk,
					postY);
			if (!Skill.isUsedSkill2) // -----------------------------
				speedWalk += SPEED;
		} else if (countFrameWalk == 1) {
			g2d.drawImage(DrawingUtility.pinkWalkL2, null, postX - speedWalk,
					postY);
			if (!Skill.isUsedSkill2)// ---------------------------------
				speedWalk += SPEED;
		}
	...
	public void animationWalkR(Graphics2D g2d, int postX, int postY) {
		...
		if (countFrameWalk == 0) {
			g2d.drawImage(DrawingUtility.pinkWalkR1, null, postX + speedWalk,
					postY);
			if (!Skill.isUsedSkill2)// -----------------
				speedWalk += SPEED;
		} else if (countFrameWalk == 1) {
			g2d.drawImage(DrawingUtility.pinkWalkR2, null, postX + speedWalk,
					postY);
			if (!Skill.isUsedSkill2)// -----------------
				speedWalk += SPEED;
		}
	...
	public void animationClimb(Graphics2D g2d, int postLadder) {
		...
		if (postYD - speedClimb > postYU) {
			...
			if (countFrameClimb == 0) {
				g2d.drawImage(DrawingUtility.pinkClimb1, null, postLadder,
						postYD - speedClimb);
				if (!Skill.isUsedSkill2)// -----------------
					speedClimb += SPEED;
			} else if (countFrameClimb == 1) {
				g2d.drawImage(DrawingUtility.pinkClimb2, null, postLadder,
						postYD - speedClimb);
				if (!Skill.isUsedSkill2)// -----------------
					speedClimb += SPEED;
			}
			...
			isDown = false;
			g2d.drawImage(DrawingUtility.pinkClimb1, null, postLadder, postYD
					- speedClimb);
		}
	...
	public void animationFillRocket(Graphics2D g2d, int x) {
		...
		if (countDelayFill < delayFill) {
			g2d.drawImage(DrawingUtility.pinkFill, null, x, postYU);
			if (!Skill.isUsedSkill2)
				countDelayFill++;
		} else {
